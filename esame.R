...
ggplot((rethink %>% filter(Displ > 0.1 & Fuel == "Tier 2 Cert Gasoline" )), aes(x = HP, y = 1/MPG)) +
...
ggplot((rethink %>% filter(Displ > 0.1 & Fuel == "Tier 2 Cert Gasoline")), aes(x = Displ, y = 1/MPG)) +
  geom_point(alpha = 0.2) +
  geom_smooth(se=FALSE)

ggplot((rethink %>% filter(Displ > 0.1 & Fuel == "Tier 2 Cert Gasoline")), aes(x = Weight, y = 1/MPG)) +
  geom_point(alpha = 0.2) +
  geom_smooth(se=FALSE)

ggplot((rethink %>% filter(Displ > 0.1 & Fuel == "Tier 2 Cert Gasoline")), aes(x = Gears, y = 1/MPG)) +
  geom_point(alpha = 0.2) +
  geom_smooth(se=FALSE)

cor((rethink %>% filter(Cylinders > 0 & Fuel == "Tier 2 Cert Gasoline"))$Cylinders,
    1/(rethink %>% filter(Cylinders > 0 & Fuel == "Tier 2 Cert Gasoline"))$MPG)
